...
describe("bounded legacy bootstrap lookup", () => {
  ...
  it("uses an exact indexed decision lookup and skips malformed or pathological IDs", () => {
    ...
    const executor = { sql<T>(strings: TemplateStringsArray, ...values: (string | number | boolean | null)[]): T[] { const query = strings.reduce((result, part, index) => result + part + (index < values.length ? "?" : ""), ""); queries.push(query); const sqliteValues = values.map((value) => typeof value === "boolean" ? (value ? 1 : 0) : value) as (string | number | null)[]; if (/^\s*(SELECT|WITH)\b/i.test(query)) return db.prepare(query).all(...sqliteValues) as T[]; db.exec(query); return []; } };
