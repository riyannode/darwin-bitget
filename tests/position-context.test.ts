...
function management(action: "HOLD" | "INCREASE" | "REDUCE" | "CLOSE" | "REVERSE", id: string, createdAt = "2026-09-14T10:00:00.000Z"): Decision {
  return { ...base, decisionId: id, cycleId: `cycle-${id}`, action, createdAt, marginAllocationPct: action === "REVERSE" ? "10" : "0", additionalMarginPct: action === "INCREASE" ? "5" : null, reductionPct: action === "REDUCE" ? "25" : action === "CLOSE" ? "100" : null, targetPositionSide: action === "REVERSE" ? "SHORT" : null, thesis: `${action} thesis`, strategyThesis: `${action} strategy` };
}
...
describe("bounded position reasoning context", () => {
  it("preserves original entry reasoning while replacing only latest management", () => {
    ...
    context = upsertPositionContext(context, management("HOLD", "hold-1", "2026-09-13T10:00:00.000Z"), "2026-09-13T10:00:00.000Z");
    context = upsertPositionContext(context, management("INCREASE", "increase-1", "2026-09-14T10:00:00.000Z"), "2026-09-14T10:00:00.000Z");
  ...
  it("isolates same-side re-entry management history by lifecycle", () => {
    let context = upsertPositionContext(null, base, experience.entryTime, experience);
    context = upsertPositionContext(context, management("HOLD", "hold-1", "2026-09-13T10:00:00.000Z"), "2026-09-13T10:00:00.000Z");
    context = upsertPositionContext(context, management("CLOSE", "close-1", "2026-09-13T11:00:00.000Z"), "2026-09-13T11:00:00.000Z");
    const secondEntry = { ...base, decisionId: "entry-2", cycleId: "cycle-entry-2", thesis: "second entry thesis", strategyThesis: "second entry strategy" };
    context = upsertPositionContext(context, secondEntry, "2026-09-14T10:00:00.000Z", { ...experience, experienceId: "experience-2", entryDecisionId: "entry-2", entryThesis: "second persisted entry thesis", entryTime: "2026-09-14T10:00:01.000Z" });
    expect(context?.entryDecisionId).toBe("entry-2");
    expect(context?.managementEvents).toEqual([]);
    expect(context?.latestManagement).toBeUndefined();
    context = upsertPositionContext(context, management("HOLD", "hold-2", "2026-09-14T11:00:00.000Z"), "2026-09-14T11:00:00.000Z");
    expect(context?.managementEvents.map((event) => event.decisionId)).toEqual(["hold-2"]);
    expect(context?.latestManagement?.decisionId).toBe("hold-2");
  });
