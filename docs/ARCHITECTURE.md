...
`/api/live/portfolio` remains provider-only and performs zero Durable Object reads. `/api/snapshot` composes one provider portfolio observation with the compact Durable Object performance ledger so the dashboard does not mix independently observed financial cards. The response exposes `performanceAccounting` with the immutable baseline, current provider equity and timestamp, external-flow audit status, verified lifecycle PnL, classified closed-trade counts, win rate, and persisted high-water drawdown state. `/api/position-context?symbol=...&positionSide=...` reads one indexed `position_context` row for persisted DARWIN reasoning.
