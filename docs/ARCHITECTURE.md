...
For each verified open position, deterministic TypeScript supplies Qwen with `currentReturnPct`, `maximumFavorableReturnPct`, `maximumFavorableReturnBasis`, `profitGivebackPct`, `timeInTradeMinutes`, and recent `priorManagementActions`. `SINCE_ENTRY` means the favorable-return peak is supported from the trade-entry lifecycle. `SINCE_FIRST_DETERMINISTIC_OBSERVATION` means a legacy position did not have a trustworthy persisted full-since-entry price series, so the peak is explicitly bounded to the first deterministic observation. Qwen uses this evidence to compare HOLD, REDUCE, and CLOSE; it does not calculate or invent these lifecycle values.
...
`/api/live/portfolio` remains provider-only and performs zero Durable Object reads. A successful response contains `source=PROVIDER_LIVE`, a provider `portfolio`, and `observedAt`, with optional `degraded`/`errors` when an optional provider read fails; it does not expose a top-level stale field. `/api/snapshot` composes one provider portfolio observation with the compact Durable Object performance ledger so the dashboard does not mix independently observed financial cards. The response exposes `performanceAccounting` with the immutable baseline, current provider equity and timestamp, external-flow audit status, verified lifecycle PnL, classified closed-trade counts, win rate, and persisted high-water drawdown state. `/api/position-context?symbol=...&positionSide=...` reads one indexed `position_context` row for persisted DARWIN reasoning.

The performance aggregate stores `totalTrades`, `openTrades`, `closedTrades`, wins/losses/breakeven, verified realized lifecycle PnL, the one-time `competitionBaselineEquity` and `performanceBaselineAt`, latest equity, explicit external-flow status, incremental peak equity, current/max performance drawdown, and up to 62 UTC daily summaries. Daily summaries store opening/latest equity, daily PnL, daily return percentage, and verified opening-trade count. A provider-verified close increments `closedTrades` even when realized-PnL enrichment is unavailable; only classified PnL contributes to wins/losses/breakeven and the win-rate denominator. It is initialized once and updated incrementally from verified provider lifecycle facts. A one-time bounded bootstrap reads up to 100 current OPEN experiences, recent journals, and targeted journal rows for their entry decision IDs; it never runs on each snapshot.
