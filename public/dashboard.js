...
function render() { renderAgent(); renderPerformance(); renderCalendar(); renderPortfolio(); renderOpenPositions(); renderDecision(); renderDecisionHistory(); renderLearning(); renderRecentTrades(); renderLatestTrade(); renderActivity(); renderTradeTable(); renderLessons(); renderPolicy(); if (judgeDemo) { const banner = $("judge-demo-banner"); banner.hidden = false; banner.dataset.scenario = snapshot.demo?.title ?? "DETERMINISTIC REPLAY"; $("judge-demo-result").textContent = `RISK GATE ${snapshot.demoRiskGate?.status ?? "—"}${snapshot.demoRiskGate?.codes?.length ? ` · ${snapshot.demoRiskGate.codes.join(" / ")}` : ""}`; } $("error").hidden = true; }
...
async function refresh() { try { const scenario = new URLSearchParams(window.location.search).get("scenario"); const path = judgeDemo && scenario ? `/api/snapshot?scenario=${encodeURIComponent(scenario)}` : "/api/snapshot"; const response = await fetch(path, { cache: "no-store" }); if (!response.ok) throw new Error(`HTTP_${response.status}`); snapshot = await response.json(); render(); } catch (error) { const banner = $("error"); banner.textContent = error instanceof Error ? error.message : "SNAPSHOT_FAILED"; banner.hidden = false; } }
