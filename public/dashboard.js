...
function renderPortfolio() {
  ...
  const fields = [["ACCOUNT EQUITY", money(portfolio?.portfolioEquity)], ["AVAILABLE MARGIN", money(portfolio?.availableMargin)], ["INITIAL MARGIN", money(portfolio?.initialMargin)], ["UNREALIZED PNL", signedMoney(portfolio?.unrealizedPnl)], ["REALIZED PNL", signedMoney(portfolio?.positionRealizedPnl)], ["FUNDING", signedMoney(portfolio?.funding)], ["FEES", signedMoney(portfolio?.fees)], ["OPEN POSITIONS", portfolio?.positions?.length ?? 0]];
  const optional = [["ACCOUNT MARGIN USED", portfolio?.accountMarginUsed], ["TOTAL POSITION MARGIN", portfolio?.positionMargin], ["CASH DIVIDEND", portfolio?.cashDividend]].filter(([, value]) => value !== undefined && value !== null && value !== "" && value !== "UNAVAILABLE");
