...
function renderDecision() {
  const decision = snapshot.latestDecision; $("decision-action").textContent = decision?.action ?? "—"; const node = $("decision"); node.replaceChildren(); node.className = decision ? "detail-grid" : "detail-grid empty";
  if (!decision) { node.textContent = "No Qwen decision recorded yet."; return; }
  [detail("SYMBOL / SIDE", `${decision.symbol} / ${decision.positionSide ?? "—"}`), detail("MARGIN", `${decision.marginAllocationPct}%`), detail("LEVERAGE", `${decision.leverage}x`), detail("CONFIDENCE", decision.confidence), detail("POSITION NOTIONAL", money(snapshot.executionEvidence?.positionNotional ?? "0")), detail("TIMESTAMP", when(decision.createdAt)), detail("STRATEGY THESIS", decision.strategyThesis, true), detail("SUPPORTING FACTORS", list(decision.supportingFactors), true), detail("RISK FACTORS", list(decision.riskFactors), true), detail("LESSONS USED", list(decision.lessonsUsed), true)].forEach((item) => node.append(item));
}
...
function render() { renderAgent(); renderPerformance(); renderCalendar(); renderPortfolio(); renderDecision(); renderLearning(); renderRecentTrades(); renderActivity(); renderTradeTable(); renderLessons(); renderPolicy(); $("error").hidden = true; }

function selectPage(page) { document.querySelectorAll(".page").forEach((item) => { item.hidden = item.id !== `page-${page}`; }); document.querySelectorAll(".nav-item").forEach((item) => item.classList.toggle("active", item.dataset.page === page)); const label = page === "learning-page" ? "Learning" : page === "policy-page" ? "Policy" : page === "trades" ? "Trade Log" : "Dashboard"; $("page-title").textContent = label; }
